...
# ---------------------------------------------------------------------------
# HyDE：检索分数低时，生成假设答案用于二次检索
# ---------------------------------------------------------------------------
HYDE_PROMPT = ChatPromptTemplate.from_messages([
    ("system", """你是医疗信息检索专家。请基于用户问题，生成一段「假设的参考答案」。

这段假设答案将被当作检索 query 使用，因此：
1. 内容专业、信息密度高，尽可能包含与问题相关的关键医学术语与实体
2. 不需要真实准确（它是假设文档），但读起来要像一篇权威医学资料的片段
3. 3~5 句话，只输出答案正文，不要任何前缀、解释或格式标记。"""),
    ("human", "问题：{question}\n\n假设答案："),
])
